...
public class Environment implements InputProcessor {
     ...
     */
    public Environment(String nnetPath, Random random, int testLevelID) {
        ...
        // Add the Trainee
        if( NNET ) { 
        	this.trainees.add(new TrainingAgent(testLevelID, NeuralNetwork.createFromFile(nnetPath), startX, startY, this.TraverseNodes, random, this.TiledMap,
        ...
        try {
            ...
            for (int cellX = 0; cellX < numCellX; cellX++) {
                ...
                for (int cellY = 37; cellY >= 0; cellY--) {
                    ...
                    try {
                    	wallTile = wallLayer.getCell(cellX, cellY).getTile().getProperties().containsKey("blocked");
                    	
                    	if (wallTile != null && !wallTile) {
                    	...
                    	System.out.println("Error with cell (" + cellX + "," + cellY + ")" );
                    }
                }
            }
